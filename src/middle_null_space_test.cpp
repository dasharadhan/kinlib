...
int main() {
    ...
    std::vector<Eigen::Matrix4d> interpolated_poses = kinlib::interpolatePoses(init_ee_g, goal_ee_g, 20);
    nullSpace::Robot panda = nullSpace::getPandaRobot();


    Eigen::VectorXd current_jnt_config(7); 
    current_jnt_config = init_jnt_val;
    std::vector<Eigen::VectorXd> full_motion_plan_result;

    for (int i = 0; i < 20 - 1; ++i)
        {   
            
            Eigen::Matrix4d start_ee_g;
            kin_solver.getFK(current_jnt_config, start_ee_g);

            Eigen::Matrix4d goal_ee_g = interpolated_poses[i + 1];  
            std::vector<Eigen::VectorXd> currect_null_range = exploreNullSpaceRange(panda, current_jnt_config);

            if (currect_null_range.empty())
            {
                std::cerr << "Null space exploration failed at segment " << i << std::endl;
                break;
            }

            int middle_index = static_cast<int>(currect_null_range.size()) / 2;
            Eigen::VectorXd middle_config = currect_null_range[middle_index];

            // calculate middle_config
            Eigen::Matrix4d middle_ee_g;
            kin_solver.getFK(middle_config, middle_ee_g);

            std::vector<Eigen::VectorXd> partial_plan_result;
            kinlib::MotionPlanResult plan_info;
            ...
            {
                std::cerr << "Planning failed at segment " << i << std::endl;
                break;
            }
            full_motion_plan_result.insert(full_motion_plan_result.end(),
                                        partial_plan_result.begin(),
                                        partial_plan_result.end());

            current_jnt_config = partial_plan_result.back();
        }
            
    
    
    saveMotionPlanToCSV(full_motion_plan_result, "null_space_joints_withNSP_full.csv");
