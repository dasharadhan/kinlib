...
// User Guided Motion Planner
  Eigen::MatrixXd recorded_demo = loadCSV<Eigen::MatrixXd>(std::string(KINLIB_RESOURCES_DIR) + "VR_ende_traj_demo1.csv");
  Eigen::MatrixXd object_poses = loadCSV<Eigen::MatrixXd>(std::string(KINLIB_RESOURCES_DIR) + "VR_obj_pos_demo1.csv");
  ...
  Eigen::MatrixXd pickup_poses = loadCSV<Eigen::MatrixXd>(std::string(KINLIB_RESOURCES_DIR) + "straightWall_bigbrick_pickup_pos.csv");
  Eigen::MatrixXd loose_poses = loadCSV<Eigen::MatrixXd>(std::string(KINLIB_RESOURCES_DIR) + "straightWall_bigbrick_loose_pos.csv");
  ...
  for(int i = 0; i < pick_up_obj_poses.size(); i++)
  {
    ...
    // new_task_instance.object_poses[0](2,3) = pick_up_obj_poses[11 - i](2,3)+0.1; 
    new_task_instance.object_poses[0] = pick_up_obj_poses[pick_up_obj_poses.size()-1 - i];
    // new_task_instance.object_poses[0](1,3) = pick_up_obj_poses[pick_up_obj_poses.size()-1 - i](1,3)+ 0.05 ; 

    new_task_instance.object_poses[0](2,3) = pick_up_obj_poses[pick_up_obj_poses.size()-1 - i](2,3)+ 0.1035; 

    // new_task_instance.object_poses[1](0,3) = loose_obj_poses[i](0,3); 
    // new_task_instance.object_poses[1](1,3) = loose_obj_poses[i](1,3);
    new_task_instance.object_poses[1] = loose_obj_poses[i];
    new_task_instance.object_poses[1](2,3) = loose_obj_poses[i](2,3)+0.1035;
    // std::cout << "1111111111111111" << std::endl;
    // std::cout << loose_obj_poses[i] << std::endl;
    // std::cout << "1111111111111111" << std::endl;
    ...
    //   {
    //     demo.guiding_poses[i][j] += demo.guiding_poses[i][j] * offset;
    //   }
    // }
...
//  add guiding pose before grasp
  Eigen::Matrix4d before_grip_pos;
  
  for (int i = 0; i < all_guiding_poses.size(); i++)
  {
       before_grip_pos = all_guiding_poses[i][0];
       before_grip_pos(2,3) += 0.15;

      all_guiding_poses[i].insert(all_guiding_poses[i].begin(), before_grip_pos);
  }
//  add guiding pose after place
  Eigen::Matrix4d after_place_pos;
  
  for (int i = 0; i < all_guiding_poses.size(); i++)
  {
       after_place_pos = all_guiding_poses[i].back();
       after_place_pos(2,3) += 0.15;

      all_guiding_poses[i].push_back(after_place_pos);
  }
  ...
  // saveMotionPlanToCSV(all_motion_plans, "VR_build_wall_offset_joints.csv");
  // saveGripperConditionToCSV(motion_plans_gripper_cond, "VR_build_wall_offset_gripper_condition.csv");
  // saveMotionPlanToCSV(all_motion_plans, "corner_wall_joints.csv");
  // saveGripperConditionToCSV(motion_plans_gripper_cond, "corner_wall_gripper_condition.csv");
  // for (int i =0; i<all_motion_plans.size(); i++){
  //   Eigen::VectorXd jnt_val(7);
  //   jnt_val = all_motion_plans[i];
  //   Eigen::Matrix4d ee_g;
  //   kin_solver.getFK(init_jnt_val, ee_g);
  //   std::cout << ee_g << std::endl;
  // }
