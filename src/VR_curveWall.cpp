...
// User Guided Motion Planner
  Eigen::MatrixXd recorded_demo = loadCSV<Eigen::MatrixXd>(std::string(KINLIB_RESOURCES_DIR) + "VR_obj_traj.csv");
  Eigen::MatrixXd object_poses = loadCSV<Eigen::MatrixXd>(std::string(KINLIB_RESOURCES_DIR) + "VR_obj_pos.csv");
  ...
  // Eigen::MatrixXd pickup_poses = loadCSV<Eigen::MatrixXd>(std::string(KINLIB_RESOURCES_DIR) + "curve_wall_pickup_pos.csv");
  // Eigen::MatrixXd loose_poses = loadCSV<Eigen::MatrixXd>(std::string(KINLIB_RESOURCES_DIR) + "curve_wall_loose_pos.csv");

  Eigen::MatrixXd loose_poses = loadCSV<Eigen::MatrixXd>(std::string(KINLIB_RESOURCES_DIR) + "corner_wall_overlap_smallbrick_loose_pos.csv");
  Eigen::MatrixXd pickup_poses = loadCSV<Eigen::MatrixXd>(std::string(KINLIB_RESOURCES_DIR) + "corner_wall_overlap_smallbrick_pickup_pos.csv");
  ...
  for(int i = 0; i < pick_up_obj_poses.size(); i++)
  {
    ...
    new_task_instance.object_poses[0](2,3) +=  0.094; 
    //baxter arm offset
    // new_task_instance.object_poses[0](2,3) +=  0.04;

    new_task_instance.object_poses[1] = loose_obj_poses[i];
    //panda arm offset

    new_task_instance.object_poses[1](2,3) +=  0.105; 
    //baxter arm offset
    // new_task_instance.object_poses[1](2,3) +=  0.07; 
  ...
  // panda arm initial joints 
  init_jnt_val  << 0., -0.7854, 0.,-2.3562, 0., 1.5708, 0.7854;
  // baxter initial joints 
  // init_jnt_val  << 0., -0.7854, 0.,1.3, 0., 0, 0.7854;
...
//  add guiding pose after place
  ...
  // saveMotionPlanToCSV(all_motion_plans, "VR_curve_wall_joints.csv");
  // saveGripperConditionToCSV(motion_plans_gripper_cond, "VR_curve_wall_gripper_condition.csv");
  saveMotionPlanToCSV(all_motion_plans, "VR_corner_wall_overlap_smallbrick_joints.csv");
  saveGripperConditionToCSV(motion_plans_gripper_cond, "VR_corner_wall_overlap_smallbrick_gripper_condition.csv");
}
